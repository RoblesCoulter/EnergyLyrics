...
def build_graph_and_model(conflict,not_conflict,t,eigen,cluster,current_k,pat_deg_thres):
    ...
    if not file_exists:
        ...
        start = time.time()
        pattern_features_new(conflict,pats,name_conflict,name_conflict)
        ...
        # bootstrap patterns
        print('bootstrapping patterns... ')
        pats = generate_patterns(nc_graph)
        print('total patterns bootstrapped:', len(pats))
        pattern_features_new(not_conflict, pats, name_not_conflict, name_not_conflict)
...
def threadize(og_df,pats,cw,name):
    cores = multiprocessing.cpu_count()
    total = len(og_df)
    inc = math.ceil(total/cores)
    processes = [mp.Process(target=pattern_feature_thread, args=(og_df.loc[i*inc:(i*inc)+inc],pats,cw,name+'_t'+str(i),name)) for i in range(cores)]
    # Run processes
    for p in processes:
        p.start()
...
def pattern_feature_thread(posts,patterns,context_words,name,sec):
    total = len(posts)
    count = 0
    inc = 100
    thres = inc
    ...
    for doc in posts:
        doc_pats,my_diversity = generate_post_patterns(clean_post(doc, context_words),True)
        doc_pats['doc_freq'] = 1
    ...
    tot_div = patterns.merge(tot_div,on='pattern',how='left')
...
def build_models(name_conflict,name_not_conflict,pat_deg_thres,conflict,not_conflict):
    ...
    c_degrees.columns = ['pattern','freq','doc_freq','div','degree','rank']
    nc_degrees.columns = ['pattern','freq','doc_freq','div','degree','rank']
...
def classify_w_model(posts, name, sec,label):
    ...
    for post in pre_posts:
        result = generate_post_patterns(clean_post(post, c_model_words))
        c_score = 0
        if len(result)>0:
        ...
        result = generate_post_patterns(clean_post(post, nc_model_words))
        nc_score = 0
        if len(result)>0:
...
def classify(c_test,nc_test,name):
...
def batter_test_args(c_train,nc_train,t_min,t_max,t_inc,e_min,e_max,e_inc,c_min,c_max,c_inc,p_min,p_max,p_inc,cv):
                ...
                for p in range(p_min,p_max,p_inc):
                    for k in range(kfold):
                        ...
                        if (len(done[done['name'] == name]) == 0):
                            ...
                            name = build_graph_and_model(c_train_now,nc_train_now,t/100000,e/100,c/100,k,p)
                            r = classify(c_test,nc_test,name)
                            r['t']=t/100000
                            r['e']=e/100
                            r['c']=c/100
                            r['k']=k
...
def get_words(model):
    new_texts = model['pattern'].apply(lambda x: nltk.word_tokenize(x))
...
def get_words_pat(pats):
    new_texts = [nltk.word_tokenize(pat) for pat in pats]
    new_texts = [item for sublist in new_texts for item in sublist]
    new_texts = list(set(new_texts))
    new_texts.remove('.+')
    return new_texts
...
def generate_post_patterns(post,div_res=False):
    my_pats = {}
    my_div = {}
    for i in range(len(post) - 1):
        if ~post[i].startswith('.+') and ~post[i + 1].startswith('.+'):  #none of the words are wildcards
            my_pats = add_to_count_dict('.+ ' + post[i + 1],my_pats)
            my_pats = add_to_count_dict( post[i] + ' .+',my_pats)
            my_div = add_diversity_word('.+ ' + post[i + 1], post[i],my_div)
            my_div = add_diversity_word(post[i] + ' .+', post[i + 1],my_div)
        else:
        ...
        if i < len(post) - 2:
            if ~post[i].startswith('.+') and ~post[i + 1].startswith('.+') and ~post[i + 2].startswith('.+'):
                my_pats = add_to_count_dict('.+ ' + post[i + 1] + ' ' + post[i + 2],my_pats)
                my_pats = add_to_count_dict( post[i] + ' .+ ' + post[i + 2],my_pats)
                my_pats = add_to_count_dict(post[i] + ' ' + post[i + 1] + ' .+',my_pats)
                my_div = add_diversity_word('.+ ' + post[i + 1] + ' ' + post[i + 2], post[i], my_div)
                my_div = add_diversity_word(post[i] + ' .+ ' + post[i + 2], post[i + 1], my_div)
                my_div = add_diversity_word(post[i] + ' ' + post[i + 1] + ' .+', post[i + 2], my_div)
            else:
                # we need to find the wildcard agh
                if post[i].startswith('.+'):
                    my_pats = add_to_count_dict('.+ ' + post[i + 1] + ' ' + post[i + 2], my_pats)
                    my_div = add_diversity_word('.+ ' + post[i + 1] + ' ' + post[i + 2], post[i], my_div)
                elif post[i + 1].startswith('.+'):
                    my_pats = add_to_count_dict(post[i] + ' .+ ' + post[i + 2], my_pats)
                    my_div = add_diversity_word(post[i] + ' .+ ' + post[i + 2], post[i + 1], my_div)
                else:
                    my_pats = add_to_count_dict(post[i] + ' ' + post[i + 1] + ' .+', my_pats)
